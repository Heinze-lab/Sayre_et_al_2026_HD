...
class GraphSource(BatchProvider):
    ...
    def __init__(self, graph_params, graph, graph_spec=None, kind=None):
        ...
        self.kind = kind
        if kind == 'synapses':
            self.graph_pre = graph[0]
            self.graph_post = graph[0]
    ...
    def setup(self):
        ...
        if kind == 'synapses':
            self.provides(self.graph_pre, self.graph_post, spec)
        else:
    ...
    def provide(self, request):
        ...
        if self.kind == 'synapses':
            pre_list = [] 
            post_list = []
            all_nodes = list(graph.nodes)
            for u,v,_ in graph.edges:
                 pre_list += all_nodes[u]
                 post_list += all_nodes[v]
