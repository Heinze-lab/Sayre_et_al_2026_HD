...
from gunpowder.graph import GraphKey, Node, Graph, Edge
...
class RasterizeGraph(BatchFilter):
    ...
    def __rasterize(
        ...
        use_fast_rasterization = False

        # Transform the locations of all nodes to be relative to the output array
        for node in list(graph_copy.nodes):
            ...
            q = list(v)
            q[0] -= data_roi.begin[0]
            q[1] -= data_roi.begin[1]
            q[2] -= data_roi.begin[2]
            v = np.array(q)
            
            v[0] = np.max([0, v[0]])
            v[0] = np.min([v[0], data_roi.shape[0]-1])
            
            v[1] = np.max([0, v[1]])
            v[1] = np.min([v[1], data_roi.shape[1]-1])

            v[2] = np.max([0, v[2]])
            v[2] = np.min([v[2], data_roi.shape[2]-1])


            # Create a new node with modified location
            new_node = Node(node.id, v, node.temporary, node.attrs.copy())

            # Remove the old node and add the new one with modified location
            graph_copy.remove_node(node)
            graph_copy.add_node(new_node)

        # Now rasterize with transformed coordinates
        for node in graph_copy.nodes:
            # skip graph outside of mask
            if mask is not None and not mask[node.location]:
                continue
            ...
            # Now the coordinates should be properly transformed and within bounds
            
            rasterized_graph[tuple(node.location.astype(int))] = c
        ...
        if settings.edges:
            for e in graph_copy.edges:
                ...
                u = graph_copy.node(e.u)
                v = graph_copy.node(e.v)

                # The locations are already transformed, just need to convert to Coordinate
                u_coord = Coordinate(u.location.astype(int))
                v_coord = Coordinate(v.location.astype(int))
