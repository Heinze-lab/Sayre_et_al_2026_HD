...
class AddPartnerVectorMap(BatchFilter):
    ...
    def __init__(self, src_points, array, radius, trg_context,
...
#        request[self.trg_points] = PointsSpec(roi=trg_roi)
    ...
    def process(self, batch, request):
        #for a in batch.graphs[self.src_points].nodes:

        src_points = batch.graphs[self.src_points]
    ...
    def __draw_partner_vectors(self, src_points, data_roi,
        ...
        src_trg_dict = {}
        for a in src_points.nodes: #for each node in the graph (so, each ground truth annotation)
            src_trg_dict[a.id] = a.location #pull the location of the node and put in a dict of all src and trgs
        edge_dict = dict(src_points.edges) #turn the edges into a dict (src:trg)
        for sr, tr in edge_dict.items(): #for each edge - DO I NEED TO REVERSE THIS???
            src_id = sr
            trg_id = tr
            src_loc = src_trg_dict[src_id] #src loc is loc of the source in the dict
            trg_loc = src_trg_dict[trg_id] #trg loc is loc of the target in the dict
            # get the voxel coordinate, 'Coordinate' ensures integer
            v = Coordinate(src_loc / voxel_size) #v is the SOURCE LOCATION!! (which... should be the POST synapse location)

            if not data_roi.contains(v): #check location in req ROI
                logger.debug(
                    "Skipping point at %s outside of requested data ROI",
                    v)
                continue

            #assert len( #now check the partnerIDs
            #    ) == 1, \
            ...
            target = trg_loc
            if not src_points.spec.roi.contains(target):
                logger.warning(
                    "target %d of %d not in target roi: %s",
                    trg_id,
                    src_id, src_points.spec.roi)
                continue
            ...
            points_p.append(v * voxel_size) #location of the source point in world space with no offset
        ...
        for ii, point_mask in enumerate(point_masks):
            target = targets[ii]
            target_vectors[0][point_mask] = target[0] - coords[0][
                point_mask]
            target_vectors[1][point_mask] = target[1] - coords[1][
                point_mask]
            target_vectors[2][point_mask] = target[2] - coords[2][
                point_mask]
