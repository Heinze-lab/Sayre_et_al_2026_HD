...
#        request[self.trg_points] = PointsSpec(roi=trg_roi)
    ...
    def process(self, batch, request):
        print("AOIDJASOIJASOIJDJOIASD:KASLDKMASLKDJDASLKJASDOILKAJSDOIASJDOIADJO")
    ...
    def __draw_partner_vectors(self, src_points, data_roi,
        ...
        src_trg_dict = {}
        for a in src_points.nodes:
            src_trg_dict[a.id] = a.location
        edge_dict = dict(src_points.edges)
        for sr, tr in edge_dict.items():
            src_id = sr
            trg_id = tr
            src_loc = src_trg_dict[src_id]
            trg_loc = stc_trg_dict[trg_id]
            # get the voxel coordinate, 'Coordinate' ensures integer
            v = Coordinate(src_loc / voxel_size)
