...
class Synapse(object):
    ...
    def __repr__(self):
        output_str = 'id: %s, seg_ids: [%s, %s], skel_ids: [%s, %s], score: %s' % (
            ...
            '{:0.2f}'.format(self.size) if self.size is not None else None)
        return output_str
...
def create_synapses_from_db(synapses_dic):
    ...
    for syn_dic in synapses_dic:
        ...
        syn.size = syn_dic.get('size', None)

        synapses.append(syn)
    return synapses


def create_synapses(sources, targets, scores=None, sizes=None):
...
def read_synapses_in_roi(directory, roi, chunk_size=None, score_thr=-1, size_thr=-1):
        ...
        if os.path.exists(filename):
            ...
            for ii, id in enumerate(ids):
                syn = Synapse(id=id, score=scores[ii],
                             ...
                             size = sizes[ii])
                if roi.contains(syn.location_post) and syn.score > score_thr and syn.size>size_thr:
                    synapses.append(syn)
...
def visualize_synapses_in_neuroglancer(s, synapses, score_thr=-1, size_thr = -1, radius=30,
    ...
    for syn in synapses:
        if syn.score is None:
            if syn.size is None:
                ...
                add_synapse = (syn.size > size_thr)
        else:
            add_synapse = (syn.score > score_thr) and add_synapse
        if not add_synapse:
            below_score += 1
        else:
            pre_site = np.flip(syn.location_pre)
            post_site = np.flip(syn.location_post)
            description = f"id: {syn.id}, pre_seg: {syn.id_segm_pre}, post_seg: {syn.id_segm_post}, score: {syn.score}, size:{syn.size}"
