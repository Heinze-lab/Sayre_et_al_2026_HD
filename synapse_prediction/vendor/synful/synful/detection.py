...
class SynapseExtractionParameters(object):
    ...
    def __init__(
            ...
            nms_radius=None,
            size_thr=None # if size (num pixels) should have a min amount
    ):
...
def __from_probmap_to_labels(probmap, threshold):
    ...
    """
    res = np.zeros_like(probmap) #.astype(bool)
...
def find_locations(probmap, parameters,
    ...
    if parameters.extract_type == 'cc':
        ...
        #conf_scores = __from_labels_to_confs(pred_labels, score_vol=probmap)
        size_scores = [x.area for x in regions]
    pred_locs = [loc.astype(np.int64) for loc in pred_locs]
    return pred_locs, scorelist, size_scores
