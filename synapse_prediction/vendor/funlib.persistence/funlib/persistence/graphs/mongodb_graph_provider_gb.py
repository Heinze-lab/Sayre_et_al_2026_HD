...
class MongoDbGraphProvider(SharedGraphProvider):
    ...
    Arguments:
       ...
       kind (''string'', optiona):
           ...
           Only used to specify if it's a synapse graph, so that we can split into pre
           and post synapses. If it's anything else, leave blank
        ...
        try:
            ...
            if mode != "w":
                if self.db_name not in self.client.list_database_names():
                    logger.warning(
                        "Opened with read mode %s, but no db with "
    ...
    def read_nodes(
        ...
        try:
            ...
            #print('gets here A'+str(join_collection))
            if join_collection is not None:
                #print("TRIES TO JOIN")
                to_join = {
                    v["id"]: v
                    for v in self.database[join_collection].find({}, {"_id": False})
                }
    ...
    def has_edges(self, roi):
        ...
        try:
            ...
            for i in range(num_chunks):
                ...
                query = {self.endpoint_names[0]: {"$in": node_ids[i_b:i_e]}}
    ...
    def read_edges(
        ...
        try:
            ...
            for i in range(num_chunks):
                ...
                i_e = min((i + 1) * query_size, len(node_ids))
                assert i_b < len(node_ids)
                endpoint_query = {self.endpoint_names[0]: {"$in": node_ids[i_b:i_e]}}
                if attr_filter:
    ...
    def get_graph(
        ...
        if self.directed:
            graph = MongoDbSubDiGraph(self, roi)
        else:
            # create the subgraph
            graph = MongoDbSubGraph(self, roi)
    ...
    def __create_edge_collection(self):
        ...
        self.edges.create_index(
            [(u, ASCENDING), (v, ASCENDING)], name="incident", unique=True
        )
    ...
    def __check_metadata(self, metadata):
        ...
        if self.directed is None:
            assert metadata["directed"] is not None, (
                "Meta collection exists but does not contain " "directed information"
            )
        ...
        if self.total_roi is None:
            if "total_roi_offset" in metadata and "total_roi_shape" in metadata:
    ...
    def __pos_query(self, roi):
        ...
        if type(self.position_attribute) == list:
            assert len(self.position_attribute) == roi.dims, (
                "Number of position attributes does not match number of " "dimensions"
            )
...
class MongoDbSharedSubGraph(SharedSubGraph):
    def __init__(self, graph_provider, roi):
        ...
        self.client = MongoClient(self.provider.host, directConnection=True)
        self.database = self.client[self.provider.db_name]
        self.nodes_collection = self.database[self.provider.nodes_collection_name]
        self.edges_collection = self.database[self.provider.edges_collection_name]
    ...
    def update_node_attrs(self, roi=None, attributes=None):
        ...
        for node_id, data in self.nodes(data=True):
            ...
            if not attributes:
                ...
                if not update:
                    logger.info(
                        "Skipping node %s with data %s"
                        " - no attributes to update" % (node_id, data)
                    )
                    continue
                update = {"$set": update}
    ...
    def update_edge_attrs(self, roi=None, attributes=None):
        ...
        for u, v, data in self.edges(data=True):
            ...
            if not attributes:
                ...
                if not update:
                    logger.info(
                        "Skipping edge %s -> %s with data %s"
                        "- no attributes to update" % (u, v, data)
                    )
                    continue
                update = {"$set": update}
    ...
    def __write_no_flags(self, collection, old_docs, new_docs):
        bulk_query = [
            ReplaceOne(old, new, upsert=True) for old, new in zip(old_docs, new_docs)
        ]
    ...
    def __write_fail_if_not_exists(self, collection, old_docs, new_docs):
        ...
        bulk_query = [
            ReplaceOne(old, new, upsert=False) for old, new in zip(old_docs, new_docs)
        ]
    ...
    def __contains(self, roi, node):
        ...
        logger.debug(
            "Checking if coordinate {} is inside roi {}".format(coordinate, roi)
        )
