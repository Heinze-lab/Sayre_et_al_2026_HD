...
def mknet(parameter, name):
    ...
    bet2=.999
    eps=1e-8
#    experiment.log_parameters({'adam_b1':bet1,
#                               'adam_b2':bet2,
#                               'adam_eps':eps})
    ...
    tf.train.export_meta_graph(filename='./checkpoints/'+name + '.meta')
    ...
    #experiment.log_parameters(names)
    with open('./checkpoints/'+name+ '_config.json', 'w') as f:
        json.dump(names, f)
...
#    mknet(parameter, name='train_net')
    ...
    #parameter['input_size'] = (90, 113, 1132)
    mknet(parameter, name=parameter['config_name'])
    # parameter['comet_key']=com_key
